...
// Deal numbers are short 4-character random codes (e.g. "A7K2").
// Confusing characters (0/O, 1/I) are excluded for readability.
const DEAL_CODE_CHARS = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';

const makeDealCode = () =>
  Array.from({ length: 4 }, () => DEAL_CODE_CHARS[Math.floor(Math.random() * DEAL_CODE_CHARS.length)]).join('');

// Generate a unique 4-character deal code, retrying on collision
const generateDealNo = async () => {
  let code = makeDealCode();
  let attempts = 0;
  // eslint-disable-next-line no-await-in-loop
  while (await Deal.findOne({ dealNo: code }).lean()) {
    code = makeDealCode();
    if (++attempts > 25) break; // extremely unlikely; avoids infinite loop
  }
  return code;
};

// @route   GET /api/deals/next-number
// @desc    Preview a fresh random 4-letter deal code for a new deal
router.get('/next-number', authMiddleware, async (req, res) => {
  try {
    const dealNo = await generateDealNo();
    res.json({ dealNo });
  } catch (err) {
    res.status(500).json({ message: 'Server error generating deal number preview' });
  }
});
...
router.get('/', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (search) {
      ...
      // Deal codes are alphanumeric strings; match by code (case-insensitive) or customer
      query.$or = [
...
router.post('/', authMiddleware, async (req, res) => {
  ...
  try {
    // Use the client-previewed code if it is a valid, unused 4-char code; otherwise generate one.
    let dealNo = typeof req.body.dealNo === 'string' ? req.body.dealNo.trim().toUpperCase() : '';
    if (!/^[A-Z0-9]{4}$/.test(dealNo) || (await Deal.findOne({ dealNo }).lean())) {
      dealNo = await generateDealNo();
    }
