...
const computeDealOutstanding = async (dealId, tranDateStr) => {
  ...
  // If no past transactions exist, interest starts accruing from the dealDate
  const startDate = (pastTransactions.length > 0 && deal.lastPaidUpto)
    ? deal.lastPaidUpto
    : (deal.dealDate || deal.dealStartDate);
  ...
  // For deals older than one year, show the amount at each anniversary and today.
  const yearlyBreakdown = [];
  const originalStartDate = deal.dealStartDate || deal.dealDate;
  const overallCalc = calculateInterest({
  ...
  for (let year = 1; year <= completedYears; year += 1) {
    ...
    const yearCalc = calculateInterest({
      startDate: originalStartDate,
...
router.post('/calculate', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      lastBalance,
      currentBalance,
      yearlyBreakdown: outstanding.yearlyBreakdown
    });
  } catch (err) {
    console.error(err);
