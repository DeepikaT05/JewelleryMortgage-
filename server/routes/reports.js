...
const { calculateInterest, getDealYearlyBreakdown } = require('../utils/calculations');
...
// ==========================================
// REPORT 2b: Customer Statement (deals + transactions + summary)
// Used by the dedicated Customer detail view (web + mobile)
// ==========================================
router.get('/customer-statement/:customerId', authMiddleware, async (req, res) => {
  const { customerId } = req.params;
  const companyId = req.user.companyId;
  ...
  try {
    ...
    const deals = await Deal.find({ customerId, companyId })
      .populate('items.groupId')
      .sort({ dealDate: -1 })
      .lean();

    // Attach a year-by-year value breakdown for deals older than a year
    deals.forEach(d => {
      d.yearlyBreakdown = getDealYearlyBreakdown(d, customer, new Date());
    });

    const transactions = await Transaction.find({ customerId, companyId })
      .populate('dealId', 'dealNo')
      .sort({ tranDate: -1 })
      .lean();

    // Summary figures
    const totalDealAmount = deals.reduce((s, d) => s + Number(d.dealAmount || 0), 0);
    const totalPrincipalPaid = transactions.reduce((s, t) => s + Number(t.principle?.amountPaid || 0), 0);
    const totalInterestPaid = transactions.reduce((s, t) => s + Number(t.compound?.amountPaid || 0), 0);
    const totalPaid = transactions.reduce((s, t) => s + Number(t.totalPaid || 0), 0);
    const activeDeals = deals.filter(d => d.status !== 'settled').length;
    const outstandingPrincipal = Math.max(0, totalDealAmount - totalPrincipalPaid);
    ...
    console.error(err);
    res.status(500).json({ message: 'Server error compiling customer statement' });
  }
});
...
// ==========================================
// REPORT 6: Period Report (custom / weekly / monthly audit)
// ==========================================
router.get('/period-report', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const [dealsBefore, txsBefore, periodDeals, periodTransactions] = await Promise.all([
      Deal.find({ companyId, dealDate: { $lt: periodStart } }),
      Transaction.find({ companyId, tranDate: { $lt: periodStart } }),
      Deal.find({ companyId, dealDate: { $gte: periodStart, $lte: periodEnd } }).populate('customerId'),
      Transaction.find({ companyId, tranDate: { $gte: periodStart, $lte: periodEnd } }).populate('customerId')
    ]);

    const GirviSetup = require('../models/GirviSetup');
    const girviSetup = await GirviSetup.findOne({ companyId });
    const storeOpeningCapital = (girviSetup && girviSetup.openingBalance !== undefined) ? girviSetup.openingBalance : 1500000;

    const LedgerTransaction = require('../models/LedgerTransaction');
    const pastLedgerTxs = await LedgerTransaction.find({
      companyId,
      refType: 'manual',
      date: { $lt: periodStart }
    });
    const pastAdd = pastLedgerTxs.filter(t => t.type === 'add').reduce((s, t) => s + (t.amount || 0), 0);
    const pastDeduct = pastLedgerTxs.filter(t => t.type === 'deduct').reduce((s, t) => s + (t.amount || 0), 0);

    const totalDealsBefore = dealsBefore.reduce((sum, deal) => sum + Number(deal.dealAmount || 0), 0);
    const totalReceivedBefore = txsBefore.reduce((sum, transaction) => sum + Number(transaction.totalPaid || 0), 0);
    const openingBalance = Math.abs(parseFloat((storeOpeningCapital + totalReceivedBefore + pastAdd - totalDealsBefore - pastDeduct).toFixed(2)));

    const events = [
      ...periodDeals.map(deal => ({
        date: deal.dealDate,
        type: 'Deal',
        customerName: deal.customerId?.name || 'Unknown',
        refNo1: deal.refNo || deal.dealNo || '',
        refNo2: deal.dealNo || '',
        principalAmt: 0,
        interestAmt: 0,
        payAmt: Number(deal.dealAmount || 0),
        balanceImpact: -Number(deal.dealAmount || 0)
      })),
      ...periodTransactions.map(transaction => ({
        date: transaction.tranDate,
        type: 'Receipt',
        customerName: transaction.customerId?.name || 'Unknown',
        refNo1: transaction.transactionNo || '',
        refNo2: transaction.transactionNo || '',
        principalAmt: Number(transaction.principle?.amountPaid || 0),
        interestAmt: Number(transaction.compound?.amountPaid || 0),
        payAmt: 0,
        balanceImpact: Number(transaction.principle?.amountPaid || 0) + Number(transaction.compound?.amountPaid || 0)
      }))
    ].sort((a, b) => new Date(a.date) - new Date(b.date));

    let runningBalance = openingBalance;
    const rows = events.map((event, index) => {
      runningBalance += event.balanceImpact;
      const { balanceImpact, ...row } = event;
      return { ...row, serial: index + 1, balance: Math.abs(parseFloat(runningBalance.toFixed(2))) };
    });

    const totals = rows.reduce((result, row) => ({
      principalAmt: result.principalAmt + row.principalAmt,
      interestAmt: result.interestAmt + row.interestAmt,
      payAmt: result.payAmt + row.payAmt
    ...
    res.json({
      ...
      rows,
      totals,
      closingBalance: Math.abs(parseFloat(runningBalance.toFixed(2)))
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error generating period report' });
  }
});

// ==========================================
// REPORT 7: Day Report (Daily Audit / Closing Report)
// ==========================================
  ...
  try {
    ...
    dayDeals.forEach(d => {
      ...
      rows.push({
        ...
        interestAmt: 0,
        payAmt: principal,
        balance: Math.abs(parseFloat(runningBalance.toFixed(2)))
      });
    });
    ...
    const closingBalance = Math.abs(parseFloat(runningBalance.toFixed(2)));
