...
// Resolve the store's configured financial year label (e.g. "2024-25")
// from the company's financialYearStart/End. Falls back to the current
// Indian FY (April-March) if not configured.
const getStoreFinancialYear = async (companyId) => {
  try {
    const company = await Company.findById(companyId);
    if (company && company.financialYearStart) {
      const s = new Date(company.financialYearStart);
      const e = new Date(company.financialYearEnd || company.financialYearStart);
      return `${s.getFullYear()}-${String(e.getFullYear() % 100).padStart(2, '0')}`;
    }
  } catch (err) {
    console.error('Error resolving store FY:', err);
  }
  // Fallback: current Indian financial year
  const now = new Date();
...
router.post('/', authMiddleware, upload.single('idProofImage'), async (req, res) => {
  ...
  try {
    ...
    const storeFY = await getStoreFinancialYear(req.user.companyId);

    const newCustomer = new Customer({
      customerCode: counter.seq,
      name,
      financialYear: storeFY,
      fatherHusbandName,
      address,
      state,
...
router.put('/:id', authMiddleware, upload.single('idProofImage'), async (req, res) => {
  if (req.user.role !== 'admin') {
    return res.status(403).json({ message: 'Permission denied: Only admin users can update customer records' });
  }
  try {
    ...
    // Tag the customer with the store's current financial year on edit
    updates.financialYear = await getStoreFinancialYear(req.user.companyId);
