...
const GirviSetup = () => {
    ...
    try {
      ...
      if (setupRes.data) {
        setGirviForm({
          ...
          openingBalanceMode: setupRes.data.openingBalanceMode || 'cash',
          openingBalanceCustomSource: setupRes.data.openingBalanceCustomSource || '',
  ...
  const [girviForm, setGirviForm] = useState({
    ...
    openingBalanceMode: 'cash',
    openingBalanceCustomSource: '',
          ...
          <form onSubmit={handleSaveGirvi} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs text-slate-300">
              ...
              <div className="space-y-4">
                ...
                {/* 7) Opening Balance & Source */}
                <div className="space-y-3 p-4 bg-slate-900/60 border border-slate-800 rounded-xl">
                  <div>
                    <label className="block text-slate-300 font-semibold mb-1">Opening Balance Source (आरंभिक बैलेंस माध्यम)</label>
                    <select
                      value={
                        ['cash', 'bank', 'both'].includes(girviForm.openingBalanceMode)
                          ? girviForm.openingBalanceMode
                          : (girviForm.openingBalanceCustomSource || 'other')
                      }
                      onChange={(e) => {
                        const val = e.target.value;
                        if (['cash', 'bank', 'both'].includes(val)) {
                          setGirviForm({ ...girviForm, openingBalanceMode: val, openingBalanceCustomSource: '' });
                        } else if (val === 'other') {
                          setGirviForm({ ...girviForm, openingBalanceMode: 'other', openingBalanceCustomSource: '' });
                        } else {
                          // Selected a saved custom source (e.g., deepika)
                          setGirviForm({ ...girviForm, openingBalanceMode: 'other', openingBalanceCustomSource: val });
                        }
                      }}
                      className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-sm text-slate-100 focus:outline-none focus:border-amber-500 font-semibold"
                    >
                      <option value="cash">Cash</option>
                      <option value="bank">Bank</option>
                      <option value="both">Cash + Bank</option>
                      {(girviForm.customSourcesList || []).map((src, idx) => (
                        <option key={idx} value={src}>{src}</option>
                      ))}
                      <option value="other">+ Add New Other Source</option>
                    </select>
                  </div>

                  {girviForm.openingBalanceMode === 'both' ? (
                    <div className="space-y-3 pt-1 border-t border-slate-800/80 animate-fade-in">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs text-emerald-400 font-semibold mb-1">Cash Opening (नकद आरंभिक)</label>
                          <input
                            type="number"
                            step="0.01"
                            placeholder="e.g. 1000"
                            value={girviForm.openingCashBalance || ''}
                            onChange={(e) => {
                              const cash = Number(e.target.value || 0);
                              setGirviForm({
                                ...girviForm,
                                openingCashBalance: cash,
                                openingBalance: cash + (girviForm.openingBankBalance || 0)
                              });
                            }}
                            className="w-full px-3 py-2 bg-slate-950 border border-emerald-500/40 rounded-lg text-sm text-slate-100 font-mono focus:outline-none focus:border-emerald-500"
                          />
                        </div>
                        <div>
                          <label className="block text-xs text-blue-400 font-semibold mb-1">Bank Opening (बैंक आरंभिक)</label>
                          <input
                            type="number"
                            step="0.01"
                            placeholder="e.g. 2000"
                            value={girviForm.openingBankBalance || ''}
                            onChange={(e) => {
                              const bank = Number(e.target.value || 0);
                              setGirviForm({
                                ...girviForm,
                                openingBankBalance: bank,
                                openingBalance: (girviForm.openingCashBalance || 0) + bank
                              });
                            }}
                            className="w-full px-3 py-2 bg-slate-950 border border-blue-500/40 rounded-lg text-sm text-slate-100 font-mono focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      </div>
                      <div className="p-2.5 bg-amber-500/10 border border-amber-500/30 rounded-lg flex justify-between items-center text-xs font-mono text-amber-300">
                        <span>Total Opening Balance (कुल आरंभिक = Cash + Bank):</span>
                        <span className="font-bold text-sm text-amber-400">
                          ₹{((Number(girviForm.openingCashBalance || 0)) + (Number(girviForm.openingBankBalance || 0))).toLocaleString('en-IN')}
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div>
                      <label className="block text-slate-300 font-semibold mb-1">7) Opening Balance (आरंभिक बैलेंस)</label>
                      <input
                        type="number"
                        step="0.01"
                  ...
                  {(girviForm.openingBalanceMode === 'other' && !girviForm.customSourcesList?.includes(girviForm.openingBalanceCustomSource)) && (
                    <div className="mt-2 animate-fade-in">
                      <label className="block text-xs text-purple-400 font-semibold mb-1">New Custom Source Name (नया माध्यम नाम)</label>
                      <input
                        type="text"
                        placeholder="e.g. deepika, Owner Capital..."
                        value={girviForm.openingBalanceCustomSource}
                        onChange={(e) => setGirviForm({ ...girviForm, openingBalanceCustomSource: e.target.value })}
                        className="w-full px-3 py-2 bg-slate-950 border border-purple-500/40 rounded-lg text-sm text-slate-100 focus:outline-none focus:border-purple-500"
                      />
                    </div>
                  )}
                </div>
