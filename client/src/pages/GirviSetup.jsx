    ...
    <div className="space-y-6">
      ...
      <div className="flex space-x-1 p-1 bg-slate-900 border border-slate-800 rounded-xl max-w-lg">
        ...
        <button
          ...
          className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all ${
            activeTab === 'sms' ? 'bg-primary-600 text-white' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          SMS Setup
        </button>
        ...
        {activeTab === 'sms' && (
          <form onSubmit={handleSaveSms} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-slate-350">
              <div>
                <label className="block text-slate-400 font-semibold mb-1">API Provider</label>
                <select
                  value={smsForm.smsApiProvider}
                  onChange={(e) => setSmsForm({ ...smsForm, smsApiProvider: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-slate-300 focus:outline-none"
                >
                  <option value="mock">Mock Sandbox Logger</option>
                  <option value="twilio">Twilio Cloud API</option>
                  <option value="msg91">Msg91 Global SMS</option>
                </select>
              </div>

              <div>
                <label className="block text-slate-400 font-semibold mb-1">Sender ID</label>
                <input
                  type="text"
              ...
              <div className="md:col-span-2">
                <label className="block text-slate-400 font-semibold mb-1">API Secret Key</label>
                <input
                  type="password"
              ...
              <div className="md:col-span-2">
                <label className="block text-slate-400 font-semibold mb-1">SMS Template text</label>
                <textarea
                  rows="3"
                  value={smsForm.autoReminderTemplate}
                  onChange={(e) => setSmsForm({ ...smsForm, autoReminderTemplate: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-sm text-slate-200 focus:outline-none leading-relaxed"
                />
              </div>

              <div className="p-3 bg-slate-950/40 border border-slate-850 rounded-xl flex items-center space-x-2 md:col-span-2">
                <input
                  type="checkbox"
                  id="smsEnable"
                  checked={smsForm.isEnabled}
                  onChange={(e) => setSmsForm({ ...smsForm, isEnabled: e.target.checked })}
                  className="rounded accent-primary-500 h-4.5 w-4.5 cursor-pointer"
                />
                <label htmlFor="smsEnable" className="font-bold text-slate-350 cursor-pointer select-none">
                  Enable Gateway SMS Alerts
                </label>
              </div>
            </div>
            ...
            <button
              type="submit"
              className="px-6 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-lg text-xs font-semibold shadow-md flex items-center space-x-1.5"
            >
              <Save className="h-4 w-4" />
              <span>Update Entries</span>
            </button>
